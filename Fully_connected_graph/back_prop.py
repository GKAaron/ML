...
class Neural_Network():
    ...
    def forward(self, X):
        ...
        '''
        h = self.sigmoid(self.fully_connected(X,self.U,self.e))
        c = self.sigmoid(self.fully_connected(h,self.W,self.f))
        return c
    ...
    def derivative_of_w(self, b, delta_k):
        ...
        '''
        k = np.sum(delta_k,axis=0).reshape((1,-1))
        b_s = np.sum(b,axis=0).reshape((-1,1))
        d_w = np.dot(b_s,k)
        return d_w
    ...
    def derivative_of_u(self, X, delta_j):
        ...
        '''
        x = np.sum(X,axis=0).reshape((-1,1))
        d_j = np.sum(delta_j,axis=0).reshape((1,-1))
        d_u = np.dot(x,d_j)
        return d_u
    ...
    def backward(self, X, gt, o):
        ...
        d_k = self.error_at_layer2(d_o,o)
