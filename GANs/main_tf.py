...
def train(model, mnist_dataset, learning_rate=0.0005, batch_size=16,
    ...
    for step in range(0, num_steps):
        ...
        batch_z = np.random.uniform(-1,1,[batch_size,4])
        # Train generator and discriminator
        model.session.run([model.dis,model.gen],
...
def main(_):
    ...
    for x_idx,_ in enumerate(x_z):
        for y_idx,_ in enumerate(y_z):
            z_mu = np.random.uniform(-1,1,[1,4])
            img = model.generate_samples(z_mu)
            out[x_idx*28:(x_idx+1)*28,
                y_idx*28:(y_idx+1)*28] = img[0].reshape(28,28)
    plt.imsave('gan_pic.png',out,cmap='gray')
