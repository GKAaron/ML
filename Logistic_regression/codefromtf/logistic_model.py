...
class LogisticModel_TF(object):
    ...
    def build_graph(self, learn_rate):
        ...
        self.W0 = tf.Variable(self.W0)
        self.X = tf.placeholder(tf.float32,[None,self.ndims+1])
        self.Y = tf.placeholder(tf.float32,[None,1])
        loss = tf.pow(self.Y-tf.sigmoid(tf.matmul(self.X,self.W0)),2)
        self.W = tf.train.GradientDescentOptimizer(learn_rate).minimize(loss)
    ...
    def fit(self, Y_true, X, max_iters):
        ...
        for i in range(max_iters):
            sess.run(self.W,{self.X:X,self.Y:Y_true})
        predict = tf.sigmoid(tf.matmul(self.X,self.W0))
        return sess.run(predict,{self.X:X}) 
