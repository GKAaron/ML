...
/cygdrive/c/Users/gouke/Downloads/circle.png
        ...
        #######################
        n = img.shape[0]
        unary_feature = np.zeros((n,2))
        ind0 = np.where(img==0)[0]
        ind1 = np.where(img==1)[0]
        ind2 = np.zeros(ind0.shape,dtype='int32')
        ind3 = np.ones(ind1.shape,dtype='int32')
        unary_feature[ind0,ind2]=1
        unary_feature[ind1,ind3]=1
        return unary_feature
    ...
    def get_pairwise_features(self):
        ...
        z = np.zeros((n,1))
        o = np.ones((n,1))
        pair_feature = np.concatenate((o,z,z,o),1)
        return pair_feature
    ...
    def calculate_unary_potentials(self, unary_features):
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        unary_po = self.unary_weight*unary_features
        return unary_po
    ...
    def calculate_pairwise_potentials(self, pairwise_features):
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        pair_po = self.pairwise_weight*pairwise_features
        return pair_po
    ...
    def build_training_obj(self, img_features, unary_beliefs, pair_beliefs,
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        g_t = self.calculate_unary_potentials(img_features)
        g_t_p = self.get_pairwise_beliefs(img_features.copy())
        g_t_p = tf.cast(g_t_p*pairwise_potentials,tf.float32)
        for u_b,p_b,u_p in zip(unary_beliefs,pair_beliefs,unary_potentials):
            obj += tf.cast(tf.reduce_sum(u_b*u_p),tf.float32) + tf.cast(tf.reduce_sum(p_b*pairwise_potentials),tf.float32)\
            -tf.reduce_sum(g_t)-tf.reduce_sum(g_t_p)
        return obj
    ...
    def inference_itr(self, unary_beliefs, unary_pots, pairwise_pots):
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        index = np.arange(unary_beliefs.shape[0])
    ...
    def calculate_local_score(self, node, assignment, unary_beliefs,
        ...
        for i in self.neighbors[node]:
            ind = self.pair_inds[(i,node)]
            ind1,ind2 = self.pairs[ind]
            if node == ind1:
                ind1 = ind2
            b1 = np.where(unary_beliefs[ind1]==1)[0][0]
            if b1 == assignment:
                if b1 == 0:
                    score += pairwise_potentials[ind,0]
                else:
                    score += pairwise_potentials[ind,3]
            else:
                if node < ind1:
                    b2 = assignment
                else:
                    b2 = b1
                if b2 == 0:
                    score += pairwise_potentials[ind,1]
                else:
                    score += pairwise_potentials[ind,2]
        return score
    ...
    def check_convergence(self, new_unary_beliefs, old_unary_beliefs,
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        n = new_unary_beliefs.shape[0]
        value = np.sum(new_unary_beliefs!=old_unary_beliefs)/(2*n)
        return value < convergence_margin
    ...
    def get_pairwise_beliefs(self, unary_beliefs):
        ...
        '''
        result = np.zeros((len(self.pairs), 4),dtype='float32')
        #######################
        #IMPLEMENT THIS METHOD#
        #######################
        i = 0
        for ind1,ind2 in self.pairs:
            ind1 = np.where(unary_beliefs[ind1]==1)[0][0]
            ind2 = np.where(unary_beliefs[ind2]==1)[0][0]
            if ind1==ind2:
                if ind1==0:
                    result[i,0]=1.0
                else:
