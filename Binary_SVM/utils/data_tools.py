...
def preprocess_data(data, process_method='default'):
    ...
    if process_method == 'raw':
        ...
        data['image'] = data['image'].reshape((-1,8*8*3))
    elif process_method == 'default':
        data['image'] = data['image']/255
        data['image'] = np.abs(data['image'] - color.gray2rgb(color.rgb2gray(data['image'])))
        data = remove_data_mean(data)
        data['image'] = data['image'].reshape((-1,8*8*3))
...
def remove_data_mean(data):
    ...
    """
    average = compute_image_mean(data)
    data['image'] = data['image'] - average
    return data
