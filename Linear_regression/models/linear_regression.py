...
class LinearRegression(LinearModel):
    ...
    def backward(self, f, y):
        ...
        l = f - y
        g = np.sum(l*self.x, 1)
        grad_w = g.reshape((-1, 1))	
        total_grad = grad_w + self.w_decay_factor*self.w
        return total_grad
    ...
    def total_loss(self, f, y):
        ...
        n = y.shape[0]
        loss = f - y
        loss = 0.5*np.sum(np.square(loss))
        total_loss = loss + 0.5*self.w_decay_factor*np.sum(np.square(self.w))
        return total_loss
